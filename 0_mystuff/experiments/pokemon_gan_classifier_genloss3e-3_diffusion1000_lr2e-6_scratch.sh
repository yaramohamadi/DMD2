export CHECKPOINT_PATH=$1
export WANDB_ENTITY=$2
export WANDB_PROJECT=$3

CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node 2 --nnodes 1 main/edm/train_edm.py \
    --generator_lr 2e-6  \
    --guidance_lr 2e-6  \
    --train_iters 10000000 \
    --output_path $CHECKPOINT_PATH/pokemon_gan_classifier_genloss3e-3_diffusion1000_lr2e-6_scratch \
    --batch_size 20 \
    --initialie_generator --log_iters 500 \
    --resolution 64 \
    --label_dim 1000 \
    --dataset_name "imagenet" \
    --seed 10 \
    --model_id $CHECKPOINT_PATH/edm-imagenet-64x64-cond-adm.pkl \
    --wandb_iters 100 \
    --wandb_entity $WANDB_ENTITY \
    --wandb_project $WANDB_PROJECT \
    --use_fp16 \
    --wandb_name "pokemon_gan_classifier_genloss3e-3_diffusion1000_lr2e-6_scratch"   \
    --real_image_path $CHECKPOINT_PATH/pokemon_lmdb \
    --dfake_gen_update_ratio 5 \
    --cls_loss_weight 1e-2 \
    --gan_classifier \
    --gen_cls_loss_weight 3e-3 \
    --diffusion_gan \
    --diffusion_gan_max_timestep 1000 \
    --delete_ckpts \
    --max_checkpoint 500 

